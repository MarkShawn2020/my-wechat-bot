...
export const handleCallMenu = async (toReply: ToReply) => {
  await toReply.say(
    [
      `== ${FULL_BOT_NAME} ==`,
      '\n',
...
export const handleCallHelp = async (toReply: ToReply, toInput: string) => {
  ...
  await toReply.say(yaml.dump(targetSubscribe, {indent: 4}))
}
...
const handleSubscribes = async (msg: Message): Promise<void> => {
